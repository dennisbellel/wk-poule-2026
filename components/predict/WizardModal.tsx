...
export default function WizardModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setTeams(ts || [])
      setPlayers(ps || [])
      if (existing) setAnswers(Object.fromEntries(existing.map((a: { question_id: string; answer: string }) => [a.question_id, a.answer])))
    }
    load()
  }, [])
  ...
  async function saveAnswer(questionId: string, answer: string) {
    ...
    if (typeof step === 'number') {
      setTimeout(() => {
        const next = step + 1
        setStep(next >= questions.length ? 'done' : next)
      }, 300)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
      <div className="bg-white rounded-t-3xl w-full max-w-lg h-[88vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 text-center">
          ...
          <p className="text-sm text-gray-600 leading-relaxed mb-1">Voordat het toernooi begint, stellen we je {tourQs.length} grote vragen over het hele toernooi.</p>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
      <div className="bg-white rounded-t-3xl w-full max-w-lg h-[88vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 text-center">
          ...
          <p className="text-sm text-gray-600 mb-6">Je grote voorspellingen staan vast. Aanpassen kan nog tot 11 juni via de Bonusvragen tab.</p>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
      <div className="bg-white rounded-t-3xl w-full max-w-lg h-[88vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <p className="text-xs text-[#aaa] mb-4">Vraag {(step as number) + 1} van {tourQs.length} · {currentQ.points_value} punten</p>
          <BonusQuestionItem
            question={currentQ}
