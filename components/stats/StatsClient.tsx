...
export default function StatsClient({
  ...
  return (
    <div>
      ...
      {/* Tabs — zelfde patroon als voorspellingen */}
        ...
        {[['feed', '📰 Feed'], ['stats', '📊 Statistieken']].map(([v, l]) => (
          <button key={v} onClick={() => setTab(v as 'feed' | 'stats')}
            className={`px-3 py-3 text-sm border-b-2 -mb-px transition-colors cursor-pointer border-0 bg-transparent ${
              tab === v ? 'border-[#1a5c38] text-[#1a5c38] font-semibold' : 'border-transparent text-[#aaa]'
            }`}>
            {l}
          </button>
        ))}
      ...
      {/* Content — max breedte gecentreerd */}
                    ...
                    {finishedMatches.map(m => (
                      <div key={m.id} className="card p-4">
                        <div className="flex items-center gap-3 mb-1">
                          <span className="text-sm font-semibold flex-1">{m.home_team?.flag} {m.home_team?.name_nl}</span>
                          <span className="heading text-xl font-extrabold text-[#1a5c38]">{m.home_ft}–{m.away_ft}</span>
                          <span className="text-sm font-semibold flex-1 text-right">{m.away_team?.name_nl} {m.away_team?.flag}</span>
                        </div>
                        <p className="text-[11px] text-[#aaa] mb-2">
                          rust {m.home_ht}–{m.away_ht} · {new Date(m.scheduled_at).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })}
                        </p>
                        <EmojiReactions matchId={m.id} reactions={reactions} currentUserId={currentUserId} />
                      </div>
                    ))}
                  </div>
                </div>
                      ...
                      <div key={item.id} className="card px-4 py-3 flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-[#ccc] mt-1">
                            {new Date(item.created_at).toLocaleTimeString('nl-NL', { hour: '2-digit', minute: '2-digit' })} · {new Date(item.created_at).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })}
                          </p>
                        </div>
                      </div>
                    ))}
